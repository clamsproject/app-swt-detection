...
class AnnotatedImage:
    ...
    def __init__(
        self, filename: str, label: str, subtype_label: str, mod: bool = False
    ):
        self.image = None
...
class FeatureExtractor(object):
    ...
    def __init__(
        ...
        if img_enc_name is None:
            ...
            self.img_encoder: backbones.ExtractorModel = backbones.model_map[
                img_enc_name
            ]()
        ...
        if pos_enc_name in ["sinusoidal-add", "sinusoidal-concat"]:
            ...
            if pos_enc_name == "sinusoidal-concat":
                self.pos_vec_lookup = self.get_sinusoidal_embeddings(
                    position_dim, pos_enc_dim
                )
                self.pos_batch_lookup = self.get_batch_sinusoidal(
                    position_dim, pos_enc_dim
                )
            elif pos_enc_name == "sinusoidal-add":
                self.pos_vec_lookup = self.get_sinusoidal_embeddings(
                    position_dim, self.img_encoder.dim
                )
                self.pos_batch_lookup = self.get_batch_sinusoidal(
                    position_dim, self.img_encoder.dim
                )
    ...
    def encode_batch_position(self, cur_times, tot_time, img_batch):
    ...
    def get_batch_feature_vectors(self, raw_img_batch, cur_times, tot_time):
        img_matrix = torch.stack(
            [self.get_img_vector(raw_img, as_numpy=False) for raw_img in raw_img_batch]
        )
        return self.encode_batch_position(cur_times, tot_time, img_matrix)
...
class TrainingDataPreprocessor(object):
    ...
    def __init__(self, model_name: str):
        if model_name is None:
            self.models = [
                FeatureExtractor(model_name)
                for model_name in backbones.model_map.keys()
            ]
        else:
    ...
    def get_stills(
        ...
        fps = (
            video_stream.average_rate.numerator / video_stream.average_rate.denominator
        )
        cur_target_frame = 0
        fcount = 0
